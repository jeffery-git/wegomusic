...
const soundEffects = {
  running: [
    ...
    { id: 'running-2', label: '跑步声 2', src: `${R2_MEDIA_BASE_URL}/sound-effects/running/running2.mp3` },    { id: 'running-3', label: '跑步声 3', src: `${R2_MEDIA_BASE_URL}/sound-effects/running/running3.mp3` },
  ],
  breathing: [
...
function App() {
  ...
  const playNextTrack = () => {
    ...
    const nextTrack = allTracks[(currentIndex + 1 + allTracks.length) % allTracks.length]
  ...
  const toggleSoundEffect = (kind, effect) => {
    ...
    if (!player) return
    try {
      let node = effectAudioNodes.current[kind]
      if (!node) {
        ...
        node = { context, gain }
        effectAudioNodes.current[kind] = node
      }
      node.gain.gain.value = 1.5
      node.context.resume?.()
    } catch {}
  ...
  return <main>
    <audio ref={runningAudio} crossOrigin="anonymous" preload="none" />
    <audio ref={breathingAudio} crossOrigin="anonymous" preload="none" />
